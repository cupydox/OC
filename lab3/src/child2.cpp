...
int main(int argc, char* argv[]) {
    ...
    sem_t* s_child2 = sem_open(sem_child_name, 0);

    if (s_parent == SEM_FAILED || s_child2 == SEM_FAILED) {
        perror("sem_open");
        return 1;
    }
    ...
    while (true) {
        ...
        std::cout << "Child2: Received data: " << data << std::endl;

        std::reverse(data.begin(), data.end());
        file << data << std::endl;

        std::cout << "Child2: Processed data: " << data << std::endl;
